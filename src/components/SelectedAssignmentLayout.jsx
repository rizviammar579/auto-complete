...
const SelectedAssignmentLayout = ({ assignment, fetchAssignments, currentFilter, regeneratingId, setRegeneratingId, show, setShow }) => {
    ...
    return (
        <div className={`${show && window.innerWidth < 1400 ? "fixed inset-0 bg-black/20 backdrop-blur-md " : ""}`}>
            ...
            <div className={`z-50 bg-white w-[350px] h-full rounded-xl p-5 w-fit m-2 mt-6 ${window.innerWidth < 1400 ? "absolute inset-x-1/12 jx:inset-x-1/6 lx:inset-x-1/4 dx:inset-x-1/3 cx:inset-x-3/8 hx:inset-x-3/7 min-w-[300px] max-h-[90vh] inset-y-5" : ""}`}
               >
                <div className='flex'>
